<?php
	...
	if (in_array($result['status']['code'], [401,402,403,429])) {
		...
		foreach ($r['Response']['View'][0]['Result'] as $result) {
			...
			$temp['geometry']['lng'] = $result['Location']['DisplayPosition']['Latitude'];
			$temp['geometry']['lat'] = $result['Location']['DisplayPosition']['Longitude'];
		...
		foreach ($result['results'] as $entry) {
			...
			$temp['houseNumber'] = $entry['components']['house_number'];
			$temp['street'] = $entry['components']['road'];
