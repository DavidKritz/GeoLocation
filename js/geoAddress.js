...
function getLocAd() {
    ...
    request.onload = function () {
        ...
        if (request.status == 200) {
            ...
            document.getElementById('test2').innerHTML = data.results[0].geometry.lng
            document.getElementById('test1').innerHTML = data.results[0].geometry.lat
...
function getHereAd() {
    ...
    var api_url = 'https://geocoder.ls.hereapi.com/6.2/geocode.json'
    var request_url = api_url
        + '?apiKey=' + apikey
        + '&searchtext=' + address
    //var request_url = 'https://geocoder.ls.hereapi.com/6.2/geocode.json?apiKey=AQV-NT6PilSIlSZduYdCI1UH3T_gpjCZ8nlsdKBVSBw&searchtext=200%20S%20Mathilda%20Sunnyvale%20CA'
    var request = new XMLHttpRequest();
    request.open('GET', request_url, true);
    ...
    request.onload = function () {
        ...
        if (request.status == 200) {
            var data = JSON.parse(request.responseText);
            document.getElementById('test1').innerHTML = data.Response.View[0].Result[0].Location.DisplayPosition.Latitude
            document.getElementById('test2').innerHTML = data.Response.View[0].Result[0].Location.DisplayPosition.Longitude
